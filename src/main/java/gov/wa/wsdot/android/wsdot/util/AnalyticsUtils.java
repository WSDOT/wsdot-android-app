    ...
    private AnalyticsUtils(Context context) {
        ...
        if (firstRun) {
            Log.d(TAG, "Analytics firstRun");
           	String apiLevel = Integer.toBinaryString(Build.VERSION.SDK_INT); // Build.VERSION.SDK_INT is only available since API level 4.
        ...
        new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... voids) {
                try {
                    ...
                    Log.d(TAG, "WSDOT Analytics trackEvent: "
                            + category + " / " + action + " / " + label + " / " + value);
                } catch (Exception e) {
                    // We don't want to crash if there's an Analytics library exception.
                    Log.w(TAG, "WSDOT Analytics trackEvent error: "
        ...
        new AsyncTask<Void, Void, Void>() {
            ...
            protected Void doInBackground(Void... voids) {
                try {
                    ...
                    Log.d(TAG, "WSDOT Analytics trackPageView: " + path);
                } catch (Exception e) {
                    // We don't want to crash if there's an Analytics library exception.
                    Log.w(TAG, "WSDOT Analytics trackPageView error: " + path, e);
                }
                return null;
            }
        }.execute();
